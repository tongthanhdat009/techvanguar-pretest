...
@if (filled($toasts))
    <div class="pointer-events-none fixed right-4 top-4 z-[70] flex w-full max-w-sm flex-col gap-3 sm:right-6 sm:top-6" data-admin-toast-stack>
        @foreach ($toasts as $toast)
            ...
            @endphp
            <div
                class="pointer-events-auto flex items-start gap-3 rounded-2xl border px-4 py-3 shadow-lg transition duration-200 {{ $isError ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700' }}"
                data-admin-toast
            ...
            >
                ...
                <button type="button" class="rounded-full p-1 transition hover:bg-black/5" data-toast-dismiss aria-label="Dismiss notification">
